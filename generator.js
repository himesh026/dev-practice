...
function buildFrontendHeader(date) {
  return [`// Type      : React Component`, `// Date      : ${date}`, `// ${"─".repeat(55)}`, ""].join("\n");
}

function buildBackendHeader(date) {
  return [`// Type      : Backend Utility`, `// Date      : ${date}`, `// ${"─".repeat(55)}`, ""].join("\n");
}
...
function writeGFGSolution(problem, code, langKey, langName, ext) {
  ...
  return { written: true, filePath: full, relativePath: rel, label: `GFG: ${problem.title} (${langName})`, task: "gfg" };
}
...
function writeFrontendComponent(code) {
  ...
  _safeWrite(full, content);
  return { written: true, filePath: full, relativePath: rel, label: "Frontend: React Component", task: "frontend" };
}
...
function writeBackendUtility(code) {
  ...
  _safeWrite(full, content);
  return { written: true, filePath: full, relativePath: rel, label: "Backend: Utility Service", task: "backend" };
}
    ...
    fs.readdirSync(dir)
      ...
      .forEach((f) => {
        ...
        try {
          ...
          if (firstLine) { const m = firstLine.match(/:\s*(.+)/); if (m) label = m[1].trim(); }
        } catch {}
        entries.push({ date, taskKey, icon, label, relativePath: `${taskKey}/${f}` });
      });
  };
...
function updateReadme() {
  ...
  const mkTable = (rows) => {
    ...
    const body   = rows.map((r) => `| ${r.date} | [\`${r.relativePath}\`](./${r.relativePath}) | ${r.label} |`).join("\n");
    return header + body + "\n";
  };
  ...
  const content = [
    ...
    `![Total Files](https://img.shields.io/badge/files-${all.length}-orange)`, "",
    "---", "",
    "## 🧩 GFG Problems (DSA Practice)", "", mkTable(gfgRows),
    "## 🎨 Frontend Components", "", mkTable(frontendRows),
    "## ⚙️ Backend Utilities", "", mkTable(backendRows),
    "---", "",
    "## 📁 Project Structure", "",
    "```",
...
module.exports = { pickTask, writeGFGSolution, writeFrontendComponent, writeBackendUtility, updateReadme, todayDate, slugify };
